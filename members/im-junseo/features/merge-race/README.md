...
`8th-COKERTHON` 참가 레포의 default branch PR merge를 트리거로 감지하고, 행사 시작 이후 default branch에 실제로 존재하는 커밋 수를 팀별 점수로 집계해 15초 레이스 연출로 보여주는 임준서 담당 기능입니다.
...
Commit Race 화면을 기본 운영 화면으로 열어둔 경우에는 화면 전환 없이 신규 merge 팀과 실제 브랜치 커밋 증가분을 4.5초간 강조하고 짧은 효과음을 재생합니다.
